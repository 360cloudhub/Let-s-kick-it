...
class ComputeYelpCircles():
	def __init__(self, county_id, category):
	...
	def recursive_circles(self, c, r, indent=""):
		if (c[0] - self.main_center[0])**2 + (c[1] - self.main_center[1])**2 >= (r+self.main_radius)**2:
			# print(indent + "Circle outside circle")
			return
		if not(self.circle_intersecting([c,r]) or self.circle_inside([c,r])):
			# print(indent + "Circle outside polygon")
			return
		metre_r = self.radius_lat2m((c[1],c[0]),(c[1]+r,c[0]))
		if metre_r<=10000:
			# print(indent + f"Radius {metre_r}m less that limit, ",end='')
			search_params = ({
			"term": "restaurants",
    		"limit": 50,
			"latitude": c[1],
			...
			if len(page) < 50:
				...
				pass
				# print("More than 1000 results. Dividing...")
		else:
			pass
			# print(indent + f"Radius {metre_r}m too large. Dividing...")
			
		x, y = self.get_hex_corners(c, r)
		for i in range(6):
			center = ((x[i] + x[i+1])/2,(y[i] + y[i+1])/2)
			self.recursive_circles(center, r/2, indent+"\t")
		self.recursive_circles(c, r/2, indent+"\t")
